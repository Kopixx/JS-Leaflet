...
// Create radius dependent on GDP function
function markerSize(GDP) {
    return (GDP);
};

// Create colour dependent on GDP function
function markerColour(GDP) {
    if (GDP > 100000) {
        return "yellow";
    } else if (GDP > 75000) {
        return "blue";
...
// Create a for loop creating a circle per country with colour dependent on GDP 
for (let i = 0; i < countries.length; i++) {
    L.circle(countries[i].location, {
        radius: markerSize(countries[i].gdp_pc),
        color: markerColour(countries[i].gdp_pc)
    }).bindPopup(`<h1>${countries[i].name}</h1> <hr> <h3>GDP per capita: ${countries[i].gdp_pc}</h3>`).addTo(myMap);
};
